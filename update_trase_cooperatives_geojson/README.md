# Tool to Combine Your Own Cooperative Data with the Trase Cocoa Cooperatives GeoJSON

This open-source Python tool allows users to integrate their own cooperative disclosure data with the official Trase Cocoa Cooperatives GeoJSON for Côte d'Ivoire.

* [View the Trase Cocoa Cooperatives GeoJSON for Côte d'Ivoire on trase.earth](https://trase.earth/explore/facilities-data/map?facilityTypeId=cote-d-ivoire-cocoa-cooperatives)

It takes a standard CSV of cooperative disclosures for cooperatives in Côte d'Ivoire from a user, cleans and normalizes the text, and performs fuzzy matching against the Trase Cocoa Cooperatives GeoJSON. It updates existing cooperatives by appending the disclosing buyer, and geographically processes brand-new cooperatives by assigning them to the correct administrative departments before adding them to the output updated GeoJSON.

## Sourcing and Preparing User Cooperative Data

Many major cocoa buyers publish their cooperative supply chain data annually. We encourage you to use this tool to integrate the latest available information into the Trase Cocoa Cooperatives GeoJSON.
...
* **Updating the Trase Cocoa Cooperatives GeoJSON:** If you wish to fetch a newer version of the Trase Cocoa Cooperatives GeoJSON in the future, find the latest [Trase Cocoa Cooperatives GeoJSON for Côte d'Ivoire on trase.earth](https://trase.earth/explore/facilities-data/map?facilityTypeId=cote-d-ivoire-cocoa-cooperatives) and click the **Download** button. Save the downloaded GeoJSON file into your local `data/` directory using the same filename.

## Folder Structure

Before running the script, clone this GitHub repository (trase-public) and ensure your working directory is structured exactly like this:

```text
trase-public/
 └── cocoa cooperatives map updater
   ├── update_trase_coops.py
   ├── requirements.txt
   └── data/
       ├── user_disclosure.csv                      <-- Your data (named precisely like this)
       ├── cote-d-ivoire-cocoa-cooperatives.geojson <-- Trase Cocoa Cooperatives GeoJSON
...
| **`LATITUDE`** | Float | Yes** | The Y-coordinate in EPSG:4326 (e.g., `6.356`). |
| **`LONGITUDE`** | Float | Yes** | The X-coordinate EPSG:4326 (e.g., `-3.909`). |
...
> * **\*\*** Coordinates (`LATITUDE` / `LONGITUDE`) are **only strictly required if the cooperative does not already exist** in the Trase Cocoa Cooperatives GeoJSON. If the script cannot find a match and coordinates are missing, that cooperative will be skipped.
...
*   **Row 1 (`OUBE`) — Tests a Roll-Forward:** This cooperative exists in the Trase Cocoa Cooperatives GeoJSON for 2019, but here it is reported for 2024 without coordinates. The script will safely copy the 2019 location data and create a new 2024 point with "CHOCO CORP" added to the buyers list.
*   **Row 2 (`2 AD`) — Tests an Exact Match:** This exactly matches ID 2 for the year 2019 in the Trase Cocoa Cooperatives GeoJSON. The script will simply append "CHOCO CORP" to its existing 2019 entry.
*   **Row 3 (`CNF`) — Tests a Brand New Cooperative:** This does not exist in the Trase Cocoa Cooperatives GeoJSON. The script will use the coordinates to calculate its department, assign it a brand new ID, and plot it on the map for 2024.
...
1. All original recorded Trase cocoa cooperatives in Côte d'Ivoire.
